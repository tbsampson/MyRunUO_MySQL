    ...
    public class MyRunUO : Timer
    {
        ...
        public static void Initialize()
        {
            if (Config.Enabled)
            {
                ...
                CommandSystem.Register("UpdateMyRunUO", AccessLevel.Administrator, new CommandEventHandler(Updatemyrunuo_OnCommand)); 	// Ingame command: [UpdateMyRunUO
                CommandSystem.Register("PublicChar", AccessLevel.Player, new CommandEventHandler(PublicChar_OnCommand));				// Ingame command: [PublicChar
                CommandSystem.Register("PrivateChar", AccessLevel.Player, new CommandEventHandler(PrivateChar_OnCommand));				// Ingame command: [PrivateChar
            }
        }
        ...
        private void TimeStamp(DateTime rightNow)
        {
           /* if (Config.LoadDataInFile)
            {
				this.m_OpLayers.Write(LineStart);
                this.m_OpLayers.Write(rightnow);
                this.m_OpLayers.Write(LineEnd);
            }
            else			*/
			//string CurExp = new ExpansionInfo.Name().ToString();
            this.ExecuteNonQuery("INSERT INTO myrunuo_updates (last_update) VALUE({0})", rightNow.ToShortDateString()); //rightNow.ToShortDateString()
			this.ExecuteNonQuery("DELETE FROM myrunuo_updates WHERE timestamp NOT IN ( SELECT timestamp FROM ( SELECT timestamp FROM myrunuo_updates ORDER BY timestamp DESC LIMIT 10 )timestamp)"); // clean up logs
        }
